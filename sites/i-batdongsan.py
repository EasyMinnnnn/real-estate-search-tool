...
_IMG_LONG  = "#limage"
_IMG_SHORT = "#limage img, .property img, img"
...
def parse(link: str, html_or_soup) -> dict:
    ...
    if not image:
        img_el = soup.select_one(_IMG_LONG)
        # #limage có thể là <img> hoặc wrapper
        if img_el:
            if img_el.name == "img":
                image = (img_el.get("src") or img_el.get("data-src") or "").strip()
            else:
                img2 = img_el.select_one("img") or soup.select_one(_IMG_SHORT)
                if img2:
                    image = (img2.get("src") or img2.get("data-src") or "").strip()
        if not image:
